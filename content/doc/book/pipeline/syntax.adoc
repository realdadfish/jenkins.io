...
The param must reflect the (partial) class name of one of the standard link:https://javadoc.jenkins-ci.org/hudson/model/Cause.html[Cause]s or a derived class that is provided by a third-party plugin.
For example: `when { triggeredBy 'SCMTrigger' }` or `when { triggeredBy 'UserIdCause' }`
